...
#
# Algorithm: Calculate the five Ichimoku chart lines using 120 days of moving
# averages. Check the close prices of the most recent candle and the second most
# recent candle, and determine if the close prices lie in or out of the cloud.
# Compare the two locations to determine most recent price action.
#
# Input: Candlesticks data, must be in the form of ("time", "open", "high",
# "low", "close", "volume", "closeTime", "assetVolume", "trades",
...
# compares candle close price relative to Ichimoku chart lines
...
ichi.compare <- function(cloud.data) {
  last.index = length(cloud.data)
  current.candle = ichi.candle.check(cloud.data[last.index,])
  prev.candle = ichi.candle.check(cloud.data[last.index-1,])
